...
import { CollectionQuery, Logic } from "./collectionQuery";

export class Collection<Type> {
  private col: CollectionReference<Entity & Type>;

  public constructor(name: string) {
    this.col = createCollection<Entity & Type>(name);
  }
  ...
  public async getSome(query?: CollectionQuery, pagination?: Pagination) {
    ...
    if (pagination) {
      items = items.slice(
        ...
        pagination.lastEvaluatedKey + pagination.count
      );
    }
    return items;
  }
  ...
  public async get(id: string): Promise<Entity & Type> {
  ...
  private hasId(query: CollectionQuery): string | undefined {
    for (const filter of query.expressions) {
      if (filter.key == "id") {
        return filter.val;
      }
    }
